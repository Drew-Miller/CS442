...
//
{
    ...
    for (;;) {
        ...
        if (ch == '\n') {
            ...
            inputStream >> point2.u.a[0]; // leading whitespace okay
            do {
                ch = inputStream.get();
            } while (ch != ',' && ch != CH_EOF && ch != '\n');
            assert(ch == ','); // in CSV file, ',' is expected

            inputStream >> point2.u.a[1]; // leading whitespace okay
            do {
                ch = inputStream.get();
            } while (ch != ',' && ch != CH_EOF && ch != '\n');
            assert(ch == '\n'); // in CSV file, '\n' is expected
...
//
{
    ...
    for (;;) {
        ...
        if (ch == '\n') {
            ...
            inputStream >> point3.u.a[0]; // leading whitespace okay
            do {
                ch = inputStream.get();
            } while (ch != ',' && ch != CH_EOF && ch != '\n');
            assert(ch == ','); // in CSV file, ',' is expected

            inputStream >> point3.u.a[1]; // leading whitespace okay
            do {
                ch = inputStream.get();
            } while (ch != ',' && ch != CH_EOF && ch != '\n');
            assert(ch == ','); // in CSV file, ',' is expected

            inputStream >> point3.u.a[2]; // leading whitespace okay
            do {
                ch = inputStream.get();
