...
//
{
    ...
    if (getUniformLocation(name, location))
        CHECK_GL(glUniform3f(location,
                             static_cast<GLfloat>(v.u.g.x),
                             static_cast<GLfloat>(v.u.g.y),
                             static_cast<GLfloat>(v.u.g.z)));
}
...
UniformColorShaderProgram::UniformColorShaderProgram(string name)
    : ShaderProgram(name)
//
...
const void UniformColorShaderProgram::start(void) const
//
...
const void EadsShaderProgram::start(void) const
{
    ...
    // 10 lights. Set all components of the uniform `light` structs
    // (see "eads_vertex_shader.glsl") for each light, as well as
    // `nLights`.
    //
